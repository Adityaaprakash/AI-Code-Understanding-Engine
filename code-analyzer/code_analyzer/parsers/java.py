"""Java language AST parser implementation using tree-sitter."""
...
from code_analyzer.parsers.java_ast import extract_java_structure
...
class JavaParser(LanguageParser):
    """Tree-sitter based AST parser for Java source files."""

    def __init__(self) -> None:
        """Initialize tree-sitter Java parser."""
        self._ts_language = TSLanguage(tsjava.language())
        self._ts_parser = TSParser(self._ts_language)

    @property
    def language(self) -> Language:
        """Return Java language identifier."""
        return Language.JAVA

    def parse(self, source_code: str, source_path: str | None = None) -> ParseResult:
        """Parse Java source code text into a ParseResult containing extracted JavaStructure.

        Args:
            source_code: The raw Java source text to parse.
            source_path: Optional source file path.

        Returns:
            ParseResult carrying Java language identity, extracted JavaStructure AST,
        ...
        extracted_structure = extract_java_structure(root_node)

        if diagnostics or root_node.has_error:
            if not diagnostics:
                diagnostics.append(
                    ParseDiagnostic(
                        message="Syntax error in Java source code",
                        line=1,
                        column=0,
