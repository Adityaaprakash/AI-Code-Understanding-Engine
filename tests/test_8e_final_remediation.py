...
async def test_active_version_must_remain_intact(provider_mock):
    ...
    lexical.add(docA)
    # mock VectorDocument by using EmbeddingResult
    ...
    assert len(res_a_after) == 1  # A searchable state has not been destructively modified.
    ...
    res_b_new = lexical.search("new", repository_id="repo1", commit_sha="commit_B")
...
async def test_deleted_data(provider_mock):
    ...
    docA = CodeChunk(
        ...
        language=Language.PYTHON,
        commit_sha="commit_A",
        source_location=SourceLocation(start_line=1, end_line=5, start_column=0, end_column=0),
        metadata={"tokens": ["oldpaymentservice"]},
    )
    lexical.add(docA)
...
async def test_new_data(provider_mock):
    ...
    # Initially active is A. B is building with new_data
    docB = CodeChunk(
        id="chunk_new",
        repository_id="repo1",
        file_path="f",
...
async def test_partial_reindex_removal_isolated(provider_mock):
    ...
    # 8C removal for B CANNOT remove active A chunk
